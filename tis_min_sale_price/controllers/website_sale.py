...
class WebsiteSaleInherit(WebsiteSale):
    ...
    def shop_payment(self, **post):
        ...
        #Use per-website fields instead of ir.config_parameter
        minimum_sale_price = website.min_sale_price or 0.0
        tax_info = website.tax_type or 'tax_excluded'

        #Redirect to Extra Info step if enabled and price is too low
        if extra_step.active:
            if minimum_sale_price:
                if tax_info == 'tax_excluded' and order and order.amount_untaxed <= minimum_sale_price:
                    values = {
                        'website_sale_order': order,
                        'post': post,
                ...
                elif tax_info == 'tax_included' and order and order.amount_total <= minimum_sale_price:
                    values = {
                        'website_sale_order': order,
                        'post': post,
                        'escape': lambda x: x.replace("'", r"\'"),
        ...
        #Check cart and address redirections
        redirection = self._check_cart(order) or self._check_addresses(order)
        if redirection:
            return redirection

        #Prepare values and render payment page
        render_values = self._get_shop_payment_values(order, **post)
        render_values['only_services'] = order.only_services if order else False

        if render_values.get('errors'):
            render_values.pop('payment_methods_sudo', None)
            render_values.pop('tokens_sudo', None)
